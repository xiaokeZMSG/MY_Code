...
#define CAN_ID_LEN_LUMINUNANCE   (0x111)
...
r_System_s g_SystemStatus = {R_RESET,R_RESET,0,0,0,0,0};
#pragma section default

extern can_frame_t CAN_RxBufStandData;

void Init_max9278_reg(void);
...
void NormalWork(void)
{
    ...
    while (g_SystemStatus.CurrentMode == R_NORMALWORK)
    {
        ...
        if(0 != R_CAN1_Test())
        {
            PrintText("****** R_CAN1_Test ERROR! ******\n");
        }
        else
        {
            PrintText("****** R_CAN1_Test OK! ******\n");
            if(CAN_ID_LEN_LUMINUNANCE == CAN_RxBufStandData.ID && g_SystemStatus.LED_Luminance != CAN_RxBufStandData.DB[0])
            {
                g_SystemStatus.LED_Luminance = CAN_RxBufStandData.DB[0];
                g_SystemStatus.refreshflag_LED = 1;
            }
        }
...
void Init_max9278_reg(void)
{
    ...
    I2C_send_data[1] = 0xCF;
    R_RIIC0_Master_Send(MAX_9278_ADDR, (uint8_t*)&I2C_send_data,sizeof(I2C_send_data));
    delay(2);
	
    I2C_send_data[0] = 0x02;
    num = 1;
    R_RIIC0_Master_Send(MAX_9278_ADDR,(uint8_t*)&I2C_send_data[0] , num);
    delay(1);
    R_RIIC0_Master_Receive(MAX_9278_ADDR, (uint8_t*)&I2C_recv_data[0], 1);
    delay(1);	
