...
*****************************************************************************/
#define CBDWRP_BUS_OFF_THRESHOLD          (10)
...
void CbdWrpInit(void);
void CbdWrpTask(void);
...
*****************************************************************************/
uint8_t   ApplBusOffStart = false;

uint32_t CanSignalStopRecvCnt=0;
uint8_t Communication_Control_PowerVol_Disabled = false;
...
#define  RX_PDUID_MAX_NUB (sizeof(CanIdDataRecv)/sizeof(CanId_Recv))
...
void vCanApp_CAN_Task(void) 
{
		...
		if(FALSE == Communication_Control_PowerVol_Disabled)
		{                 
	 	    Com_MainFunctionRx();	
			if(!ApplBusOffStart)
			{
...
{
	uint8_t id_index;
	for(id_index = 0; id_index < RX_PDUID_MAX_NUB; id_index++)
	{
...
static void prvCan_IPK_0x3F1(void *ComIPduDataPtr)
{
	...
	IPKTotalOdometer = IPKTotalOdometer/10;
...
static void prvCan_GW_ESC_0x125(void *ComIPduDataPtr)
{
	...
	if(veh_data.VehicleSpeed !=VehicleSpeed ) 
	{
		 ...
		 if(veh_data.VehicleSpeed <3)
		 {
		    Vehicle_at_standstill_flag = true;
		 }
		 else
		 {
		    Vehicle_at_standstill_flag = FALSE;
		 }
	}
...
//	
	...
	Communication_Control_PowerVol_Disabled = FALSE;
    CbdWrpInit();
}
...
{
	extern void extern_releaseExternalRxTxBuffers(void);
	if(temp == 0)//bus off
	{
...
void Control_vol_Communication_enable(void)
{
    Communication_Control_PowerVol_Disabled = FALSE;
}
void Control_vol_Communication_disable(void)
{
    Communication_Control_PowerVol_Disabled = true;
}



#define CbdWrpTimerOff                        ((uint16_t)0u)
#define CBDWRP_BUS_OFF_RECOVERY_TIME_FAST      (5)
#define CBDWRP_BUS_OFF_RECOVERY_TIME_SLOW      (100)

/* BusOff recovery timer. */
uint16_t  cbdWrpBusOffRecTimer;
uint16_t  cbdWrpBusOffThresholdCounter;
...
void CbdWrpInit(void)
{
    ...
    cbdWrpBusOffRecTimer = CbdWrpTimerOff;
    cbdWrpBusOffThresholdCounter = CBDWRP_BUS_OFF_THRESHOLD;
...
void CbdWrpTask(void)
{
    ...
    if( cbdWrpBusOffRecTimer != CbdWrpTimerOff )
    {

      /* Ongoing BusOff recovery */
      cbdWrpBusOffRecTimer--;

      /* End of BusOff recovery? */
      if(cbdWrpBusOffRecTimer == 0u)
      {
         R_CAN_Init(0,0);     
         CanIf_SetPduMode( 0, CANIF_SET_ONLINE );
...
void CbdWrpBusOff( void )
{
    ...
    //  ApplCbdWrpBusOffStart();
      ApplBusOffStart = 1;

      if( cbdWrpBusOffThresholdCounter > 0u )
      {
        cbdWrpBusOffThresholdCounter--;
        cbdWrpBusOffRecTimer = CBDWRP_BUS_OFF_RECOVERY_TIME_FAST;

      }
      else
      {
        cbdWrpBusOffRecTimer = CBDWRP_BUS_OFF_RECOVERY_TIME_SLOW;
      }
