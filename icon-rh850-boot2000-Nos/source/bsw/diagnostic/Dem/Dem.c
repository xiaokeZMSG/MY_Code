...
#include "NvM.h"
#elif !defined(USE_RTE)
/* NvM_BlockIdType is defined in the RTE when used or in NvM when used
 * If neither is used, typedef it here since used by DEM
 */
...
#define DEM_EXT_DATA_IN_PRI_MEM (DEM_MAX_NUMBER_EXT_DATA_PRI_MEM > 0)
#define DEM_EXT_DATA_IN_SEC_MEM (DEM_MAX_NUMBER_EXT_DATA_SEC_MEM > 0)
#define DEM_FF_DATA_IN_PRE_INIT (DEM_MAX_NUMBER_FF_DATA_PRE_INIT > 0)
#define DEM_FF_DATA_IN_PRI_MEM (DEM_MAX_NUMBER_FF_DATA_PRI_MEM > 0)
#define DEM_FF_DATA_IN_SEC_MEM (DEM_MAX_NUMBER_FF_DATA_SEC_MEM > 0)
//20160709,xy,for fe-7
//#define DEM_DEFAULT_EVENT_STATUS (DEM_TEST_NOT_COMPLETED_SINCE_LAST_CLEAR | DEM_TEST_NOT_COMPLETED_THIS_OPERATION_CYCLE)
#define DEM_DEFAULT_EVENT_STATUS (DEM_TEST_NOT_COMPLETED_SINCE_LAST_CLEAR)
//20160709,xy,for fe-7
#define DEM_UDS_TEST_FAILED_TRESHOLD  (sint32_t)127
#define DEM_UDS_TEST_PASSED_TRESHOLD (sint32_t)(-128)
#define DEM_UDS_FDC_RANGE (sint32_t)(DEM_UDS_TEST_FAILED_TRESHOLD - (DEM_UDS_TEST_PASSED_TRESHOLD))
...
#define GET_STORED_STATUS_BITS(_x) ((_x) & (DEM_TEST_FAILED_SINCE_LAST_CLEAR | DEM_TEST_NOT_COMPLETED_SINCE_LAST_CLEAR |DEM_CONFIRMED_DTC))//20160709,xy,for FE-7
#endif
...
boolean DemEnableConditions[DEM_NUM_ENABLECONDITIONS];
#endif

uint32_t dtc_set_mask = 0;

uint32_t flash_store_event = 0x0;
static uint32_t flash_store_frame = 0;
static uint32_t flash_store_extend = 0;
static uint32_t flash_store_aged = 0;

uint8_t secAcssAttempts[4]={0x00,0x00,0x00,0x00};


/*
 * Local functions
 * */
static void getPidData(const Dem_PidOrDidType **const*pidClassPtr, FreezeFrameRecType **freezeFrame, uint16_t *storeIndexPtr);
static void getDidData(const Dem_PidOrDidType **const*didClassPtr, FreezeFrameRecType **freezeFrame, uint16_t *storeIndexPtr);
...
static boolean storeOBDFreezeFrameDataMem(const Dem_EventParameterType *eventParam, const FreezeFrameRecType *freezeFrame,
                                          FreezeFrameRecType* freezeFrameBuffer, uint32_t freezeFrameBufferSize,
                                          Dem_DTCOriginType origin);
#endif
...
static boolean deleteFreezeFrameDataMem(bool flash,const Dem_EventParameterType *eventParam, Dem_DTCOriginType origin);
static boolean deleteExtendedDataMem(bool flash,const Dem_EventParameterType *eventParam, Dem_DTCOriginType origin);
#endif
...
#ifdef DEM_USE_MEMORY_FUNCTIONS
static boolean eventIsStoredInMem(Dem_EventIdType eventId, EventRecType* eventBuffer, uint32_t eventBufferSize)
{
    boolean eventIdFound = FALSE;
    uint16_t i;

    for (i = 0; (i < eventBufferSize) && (!eventIdFound); i++) {
 ...
 */
static boolean checkDtcGroup(uint32_t dtc, const Dem_EventParameterType *eventParam)
{
    boolean result = FALSE;
...
static boolean checkDtcSeverityMask(Dem_DTCSeverityType dtcSeverityMask, const Dem_EventParameterType *eventParam)
{
    ...
    (void)eventParam;
    //lint +estring(920,pointer)  /* cast to void */
    // IMPROVEMENT: This function is optional, may be implemented here.
...
static sint8_t fdcInternalToUDSFdc(sint16_t fdcInternal, const Dem_PreDebounceCounterBasedType* pdVars)
{
    /* Map the internal counter to the corresponding UDS fdc. I.e. map from [FailedThreshold, PassedThreshold] to [-128, 127]. */
    sint32_t pdRange = (sint32_t)(pdVars->FailedThreshold - pdVars->PassedThreshold);
    sint32_t temp = DEM_UDS_FDC_RANGE*((sint32_t)((sint32_t)fdcInternal - (sint32_t)pdVars->PassedThreshold)) + (DEM_UDS_TEST_PASSED_TRESHOLD*pdRange);
    return (sint8_t)(temp/pdRange);
}

static sint8_t getDefaultUDSFdc(Dem_EventIdType eventId)
{
    sint8_t udsFdc = 0;
...
static Dem_EventStatusType preDebounceCounterBased(Dem_EventStatusType reportedStatus, EventStatusRecType* statusRecord) {
    ...
    case DEM_EVENT_STATUS_PREFAILED:
        if (statusRecord->fdcInternal < pdVars->FailedThreshold) {
            ...
            if (((sint32_t)statusRecord->fdcInternal + (sint32_t)pdVars->IncrementStepSize) < pdVars->FailedThreshold) {
    ...
    case DEM_EVENT_STATUS_PREPASSED:
        if (statusRecord->fdcInternal > pdVars->PassedThreshold) {
            ...
            if (((sint32_t)statusRecord->fdcInternal - (sint32_t)pdVars->DecrementStepSize) > pdVars->PassedThreshold) {
...
static void updateEventStatusRec(const Dem_EventParameterType *eventParam, Dem_EventStatusType eventStatus, boolean createIfNotExist, EventStatusRecType *eventStatusRec)
{
    ...
    if (eventStatusRecPtr != NULL) {
        ...
        if (eventStatus == DEM_EVENT_STATUS_FAILED) {
            ...
            dtc_set_mask |= 1 << eventStatusRecPtr->eventId;
			//---20160709,XY,FOR FE-7
            ...
            eventStatusRecPtr->eventStatusExtended |= (DEM_TEST_FAILED | DEM_TEST_FAILED_THIS_OPERATION_CYCLE | DEM_TEST_FAILED_SINCE_LAST_CLEAR);
			//---20160709,XY,FOR FE-7
            ...
            //NOTE: Warning indicator light problem review
        ...
        if (eventStatus == DEM_EVENT_STATUS_PASSED) {
            ...
            eventStatusRecPtr->eventStatusExtended &= (Dem_EventStatusExtendedType)~(DEM_TEST_NOT_COMPLETED_SINCE_LAST_CLEAR | DEM_TEST_NOT_COMPLETED_THIS_OPERATION_CYCLE);
            //IMPROVEMENT: Warning indicator light problem review
            if ((eventParam->DTCClassRef != NULL) && (eventParam->DTCClassRef->DTCKind == DEM_DTC_KIND_EMISSION_REL_DTCS)) {
...
static boolean mergeEventStatusRec(const EventRecType *eventRec)
{
    ...
    if (eventStatusRecPtr != NULL) {
        ...
        eventStatusRecPtr->eventStatusExtended |= (Dem_EventStatusExtendedType)(eventRec->eventStatusExtended & (DEM_CONFIRMED_DTC));
		//---20160709,XY,FOR FE-7
...
static void resetEventStatusRec(const Dem_EventParameterType *eventParam)
{
    ...
    if (eventStatusRecPtr != NULL) {
        // Reset event record
        sint8_t startUdsFdc = getDefaultUDSFdc(eventParam->EventID);
...
static void getEventStatusRec(Dem_EventIdType eventId, EventStatusRecType *eventStatusRec)
{
    ...
    lookupEventStatusRec(eventId, &eventStatusRecPtr);// Lookup event ID

    if (eventStatusRecPtr != NULL) { // Copy the record
...
//	agingRecFound = lookupAgingRecMem(eventParameter->EventID, (const HealingRecType **)(&agingRecLocal), DEM_DTC_ORIGIN_PRIMARY_MEMORY);
    lookupAgingRecMem(eventParameter->EventID, (const HealingRecType **)(&agingRecLocal), DEM_DTC_ORIGIN_PRIMARY_MEMORY);

    if( DEM_EVENT_ID_NULL != eventStatusRec.eventId ) {
        if( DEM_OCCCTR == elementType ) {
           // buf[0] = eventStatusRec.occurrence >> 8;//20160321
           // buf[1] = eventStatusRec.occurrence;
            buf[0] = eventStatusRec.occurrence;
        }
        else if( DEM_FAULTDETCTR == elementType ) {
            buf[0] = eventStatusRec.UDSFdc;
        }else if (DEM_AGED == elementType ) {
        	if (agingRecLocal != NULL)
        		buf[0] = agingRecLocal->agedCounter;
			else 
				buf[0] = 0;
        }else if (DEM_AGING == elementType) {
        	if (agingRecLocal != NULL)
				buf[0] = agingRecLocal->agingCounter;
			else 
				buf[0] = 0;
 ...
 */
static boolean lookupDtcEvent(uint32_t dtc, EventStatusRecType **eventStatusRec)
{
...
static uint8 getEventPriority(Dem_EventIdType eventId)
{
    ...
    while( !eventIdParamList[indx].Arc_EOL ) {
        if( eventIdParamList[indx].EventID == eventId ) {
            priority = eventIdParamList[indx].EventClass->EventPriority;
            break;
        }
        indx++;
    }
...
static Std_ReturnType getNofStoredNonOBDFreezeFrames(const Dem_EventParameterType *eventStatusRec, Dem_DTCOriginType origin, uint8_t *nofStored)
{
    uint8_t nofFound = 0;
    ...
    for( i = 0; (i < bufferSize) && (ffBufferPtr != NULL); i++) {
...
static Std_ReturnType getNextFFRecordNumber(const Dem_EventParameterType *eventParam, uint8_t *recNum, Dem_FreezeFrameKindType ffKind, Dem_DTCOriginType origin)
{
...
static boolean isValidRecordNumber(const Dem_EventParameterType *eventParam, uint8 recordNumber)
{
    ...
    if( NULL != eventParam ) {
        for( i = 0; (i < eventParam->MaxNumberFreezeFrameRecords) && !isValid; i++ ) {
                                                ...
                                                boolean* eventHandled, Dem_DTCOriginType origin, boolean removeOldFFRecords)
{
    ...
    for( j = 0; j < freezeFrameBufferSize; j++ ) {
    ...
    for( k = 0; k < DEM_MAX_NUMBER_FF_DATA_PRE_INIT; k++ ) {
    ...
    if( 0 < nofFFToMove) {
        ...
        for( offset = 0; offset < (uint16)nofFFToMove; offset++ ) {
            recordToFind = eventParam->FreezeFrameRecNumClassRef->FreezeFrameRecordNumber[findRecordStartIndex + offset];
            ffStored = FALSE;
            for( indx = 0; (indx < DEM_MAX_NUMBER_FF_DATA_PRE_INIT) && !ffStored; indx++ ) {
        ...
        if( nofFFToMove < nofStoredPreInit ) {
            ...
            for( indx = 0; indx < DEM_MAX_NUMBER_FF_DATA_PRE_INIT; indx++ ) {
...
Std_ReturnType lookupExtDataForDisplacement(const Dem_EventParameterType *eventParam, ExtDataRecType *extDataBufPtr, uint16 bufferSize, ExtDataRecType **extData )
{
    ...
    if( DEM_EVENT_ID_NULL != eventToRemove ) {
        ...
        for (uint16 indx = 0; (indx < bufferSize) && (E_OK != ret); indx++) {
...
#if ( DEM_EXT_DATA_IN_PRE_INIT || DEM_EXT_DATA_IN_PRI_MEM || DEM_EXT_DATA_IN_SEC_MEM )
static boolean StoreExtDataInMem( const Dem_EventParameterType *eventParam, ExtDataRecType *extDataMem, uint16_t bufferSize, Dem_DTCOriginType origin, boolean overrideOldData) {
	...
	#if (DEM_FLASH_STORE_ON_END == STD_ON)
		flash_store_extend |= 1<<(extData->eventId);
	#else
		osal_nv_dtc_ext_write( extData->eventId,  sizeof(ExtDataRecType), extData );
		//OS_Sleep(10);
	#endif
...
static Std_ReturnType storeEventMem(const Dem_EventParameterType *eventParam, const EventStatusRecType *eventStatus,
        EventRecType* buffer, uint32_t bufferSize, Dem_DTCOriginType origin)
{
    ...
    uint16_t i;
    // Lookup event ID
    for (i = 0; (i < bufferSize) && (!positionFound); i++){
...
static boolean deleteEventMem(bool flash,const Dem_EventParameterType *eventParam, EventRecType* eventMemory, uint32_t eventMemorySize, Dem_DTCOriginType origin)
{
    boolean eventIdFound = FALSE;
 //   uint16_t i;
	EventStatusRecType *eventStatusRecPtr;
	...
	eventStatusRecPtr->eventId 			= (configSet->EventParameter)[eventParam->EventID -1].EventID;;
	eventStatusRecPtr->eventParamRef 	= &eventIdParamList[eventParam->EventID -1];
	if (flash) {
		#if (DEM_FLASH_STORE_ON_END == STD_ON)
			flash_store_event |= 1<<(eventParam->EventID );
		#else
			osal_nv_write( DTC_NV_EVENT(eventParam->EventID),  sizeof(EventStatusRecType), (void *)eventStatusRecPtr );
			//OS_Sleep(10);
		#endif
		osal_nv_write( DTC_NV_EVENT(eventParam->EventID),  sizeof(EventStatusRecType), (void *)eventStatusRecPtr );//add by mgd on 20210727
...
static Std_ReturnType storeEventEvtMem(const Dem_EventParameterType *eventParam, const EventStatusRecType *eventStatus)
{
	...
	#if (DEM_FLASH_STORE_ON_END == STD_ON)
		flash_store_event |= 1<<(eventStatus->eventId );
		ret = E_OK;
	#else 
		ret = osal_nv_write( DTC_NV_EVENT(eventStatus->eventId),  sizeof(EventStatusRecType), (void *)eventStatus );
	#endif
	   ret = osal_nv_write( DTC_NV_EVENT(eventStatus->eventId),  sizeof(EventStatusRecType), (void *)eventStatus );/*add by mgd on 20210715*/
...
static boolean deleteExtendedDataMem(bool flash,const Dem_EventParameterType *eventParam, Dem_DTCOriginType origin)
{
    ...
    if (eventIdFound) {// Yes, clear record
        memset(&extBuffer[i-1], 0, sizeof(ExtDataRecType));
		
		if (flash) {
		#if (DEM_FLASH_STORE_ON_END == STD_ON)
			flash_store_extend |= 1<<(i);
		#else
			osal_nv_write( DTC_NV_EXT(i),  sizeof(ExtDataRecType), &extBuffer[i-1] );
			OS_Sleep(10);
		#endif
 ...
 */
static boolean lookupExtendedDataRecNumParam(uint8_t extendedDataNumber, const Dem_EventParameterType *eventParam, Dem_ExtendedDataRecordClassType const **extDataRecClassPtr, uint16_t *posInExtData)
{
...
static boolean lookupExtendedDataMem(Dem_EventIdType eventId, ExtDataRecType **extData, Dem_DTCOriginType origin)
{
    ...
    for (i = 0; (i < extBufferSize) && (!eventIdFound); i++) {// Lookup corresponding extended data
        eventIdFound = (extBuffer[i].eventId == eventId);
    }

    if (eventIdFound) {// Yes, return pointer
...
static void deleteAgingRecMem(const Dem_EventParameterType *eventParam, Dem_DTCOriginType origin)
{
    ...
    for (i = 0; i<agingBufferSize; i++){
        if (agingBuffer[i].eventId == eventParam->EventID){
			aged_cnt = agingBuffer[i].agedCounter;
            memset(&agingBuffer[i], 0, sizeof(HealingRecType));
			agingBuffer[i].agedCounter = aged_cnt++;
			
			osal_nv_write(DTC_NV_AGE(eventParam->EventID),  sizeof(HealingRecType), &agingBuffer[i] );
            agingRecDeleted = TRUE;
        }
    }

    if( agingRecDeleted ) {
    	//setAgingBlockChanged(origin); Xiaojun modify
    }
}
#endif
                                       ...
                                       Dem_DTCOriginType  origin)
{
    ...
    if (eventIdFound) {//modify by mgd on 20210727
		OccurrenceCounter = freezeFrameBuffer[i-1].data.Snapshot.OccurrenceCounter;
	    //memcpy(&freezeFrameBuffer[i-1], freezeFrame, sizeof(FreezeFrameRecType));
	    freezeFrameBuffer[i-1].data.Snapshot.ECUVoltage = freezeFrame->data.Snapshot.ECUVoltage;
		freezeFrameBuffer[i-1].data.Snapshot.TheOdometerOfLastMalfunction[0] = freezeFrame->data.Snapshot.TheOdometerOfLastMalfunction[0];
		freezeFrameBuffer[i-1].data.Snapshot.TheOdometerOfLastMalfunction[1] = freezeFrame->data.Snapshot.TheOdometerOfLastMalfunction[1];
		freezeFrameBuffer[i-1].data.Snapshot.TheOdometerOfLastMalfunction[2] = freezeFrame->data.Snapshot.TheOdometerOfLastMalfunction[2];
		freezeFrameBuffer[i-1].data.Snapshot.VehicleSpeed[0] = freezeFrame->data.Snapshot.VehicleSpeed[0];
		freezeFrameBuffer[i-1].data.Snapshot.VehicleSpeed[1] = freezeFrame->data.Snapshot.VehicleSpeed[1];
		if(freezeFrameBuffer[i-1].data.Snapshot.OccurrenceCounter < 0xff)
		{
			OccurrenceCounter++;
		} 
		freezeFrameBuffer[i-1].data.Snapshot.OccurrenceCounter = OccurrenceCounter;
        ffUpdated = TRUE;
    }
    else {
        ...
        if (eventIdFreePositionFound) {				
	     memcpy(&freezeFrameBuffer[i-1], freezeFrame, sizeof(FreezeFrameRecType));			
			...
			#else
            /* @req DEM402*/ /* Req is not the Det-error.. */
            DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_STORE_FF_DATA_MEM_ID, DEM_E_MEM_FF_DATA_BUFF_FULL);
			#endif
        }
    }

    #if (DEM_FLASH_STORE_ON_END == STD_ON)
		flash_store_frame |= 1<<(freezeFrameBuffer[i-1].eventId);
	#else
		if(0 == osal_nv_write( DTC_NV_FREEZE(freezeFrameBuffer[i-1].eventId), sizeof(FreezeFrameRecType), &freezeFrameBuffer[i-1]))
		ffUpdated = true;
		OS_Sleep(10);
	#endif
	   /*add by mgd on 20210715*/
	   if(0 == osal_nv_write( DTC_NV_FREEZE(freezeFrameBuffer[i-1].eventId), sizeof(FreezeFrameRecType), &freezeFrameBuffer[i-1]))
	   {
		   ffUpdated = true;
	   }
...
static boolean deleteFreezeFrameDataMem(bool flash,const Dem_EventParameterType *eventParam, Dem_DTCOriginType origin)
{
	...
	if (ffDeleted && flash) {
		...
		#if (DEM_FLASH_STORE_ON_END == STD_ON)
			flash_store_frame |= 1<<(i);
		#else
			osal_nv_write( DTC_NV_FREEZE(i), sizeof(FreezeFrameRecType), &freezeFrameBuffer[i-1]);
			OS_Sleep(10);
		#endif
    	osal_nv_write( DTC_NV_FREEZE(i), sizeof(FreezeFrameRecType), &freezeFrameBuffer[i-1]);
	}
 ...
 */
static boolean lookupFreezeFrameDataRecNumParam(uint8_t recordNumber, const Dem_EventParameterType *eventParam, Dem_FreezeFrameClassType const **freezeFrameClassPtr)
{
    boolean recNumFound = FALSE;
    uint8_t i ;

    if ( (NULL != eventParam->FreezeFrameClassRef) && (NULL != eventParam->FreezeFrameRecNumClassRef)) {
        for( i = 0; (i < eventParam->MaxNumberFreezeFrameRecords) && !recNumFound; i++ ) {
            if( eventParam->FreezeFrameRecNumClassRef->FreezeFrameRecordNumber[i] == recordNumber ) {
                recNumFound = TRUE;
 ...
 */
static boolean lookupFreezeFrameDataSize(uint8_t recordNumber, Dem_FreezeFrameClassType const  **freezeFrameClassPtr, uint16_t *dataSize)
{
    Std_ReturnType callbackReturnCode;
    boolean dataSizeFound = TRUE;
    uint16_t dataSizeLocal = 0;
    uint16_t i;
 ...
 */
static boolean getFreezeFrameRecord(Dem_EventIdType eventId,uint8_t recordNumber, FreezeFrameRecType **freezeFrame, Dem_FreezeFrameClassType const *FFDataRecordClass,
                                    Dem_DTCOriginType dtcOrigin, uint8_t* destBuffer, uint16_t*  bufSize, uint16_t *FFDataSize)
{
    ...
    FreezeFrameRecType* freezeFrameBuffer = NULL;
    uint32_t 				freezeFrameBufferSize = 0;
    uint8_t 				FFIdNumber; 
...
#if 0 /*modify by mgd on20210715*/

    destBuffer[0] = recordNumber;
    destBuffer[1] = 0;
    for (FFIdNumber = 0; FFDataRecordClass->FFIdClassRef[FFIdNumber]->Arc_EOL == FALSE; FFIdNumber++) {
        destBuffer[1]++;
    }

    memcpy(&destBuffer[2], (char*)&(*freezeFrame)->data, *FFDataSize); /** @req DEM071 */
    *bufSize = *FFDataSize + 2;
    ...
    destBuffer[2]  = 0xdf;
	destBuffer[3]  = 0x00;
	destBuffer[4]  = (*freezeFrame)->data.Snapshot.ECUVoltage;
	destBuffer[5]  = 0xdf;
	destBuffer[6]  = 0x01;
	destBuffer[7]  = (*freezeFrame)->data.Snapshot.VehicleSpeed[0];
	destBuffer[8]  = (*freezeFrame)->data.Snapshot.VehicleSpeed[1];
	destBuffer[9]  = 0xdf;
	destBuffer[10] = 0x02;
	destBuffer[11] = (*freezeFrame)->data.Snapshot.OccurrenceCounter;
	destBuffer[12] = 0xdf;
	destBuffer[13] = 0x03;
	destBuffer[14] = (*freezeFrame)->data.Snapshot.TheOdometerOfFirstMalfunction[0];
	destBuffer[15] = (*freezeFrame)->data.Snapshot.TheOdometerOfFirstMalfunction[1];
	destBuffer[16] = (*freezeFrame)->data.Snapshot.TheOdometerOfFirstMalfunction[2];
	destBuffer[17] = 0xdf;
	destBuffer[18] = 0x04;
	destBuffer[19] = (*freezeFrame)->data.Snapshot.TheOdometerOfLastMalfunction[0];
	destBuffer[20] = (*freezeFrame)->data.Snapshot.TheOdometerOfLastMalfunction[1];
	destBuffer[21] = (*freezeFrame)->data.Snapshot.TheOdometerOfLastMalfunction[2];
	*bufSize = 22;
#endif
    return TRUE;
...
static boolean enableConditionsSet(const Dem_EventClassType *eventClass)
{
    ...
    if( NULL != eventClass->EnableConditionGroupRef ) {
        ...
        for( i = 0; i < enableConditionGroupPtr->nofEnableConditions; i++ ) {
...
static Std_ReturnType handleEvent(Dem_EventIdType eventId, Dem_EventStatusType eventStatus)
{
    ...
    if (eventParam != NULL) {
        if (eventParam->EventClass->OperationCycleRef < DEM_OPERATION_CYCLE_ID_ENDMARK) {
            if (operationCycleStateList[eventParam->EventClass->OperationCycleRef] == DEM_CYCLE_STATE_START) {
				if (disableDtcSetting.settingDisabled)
					return E_NOT_OK;
				if (checkDtcGroup(disableDtcSetting.dtcGroup, eventParam))
					return E_NOT_OK;
				if (checkDtcKind(disableDtcSetting.dtcKind, eventParam))
					return E_NOT_OK;
				if (enableConditionsSet(eventParam->EventClass))  {/* @req DEM447 */
                    lookupEventStatusRec(eventParam->EventID, &eventStatusBeforeUpdate);
                    ...
                    if (eventStatusLocal.errorStatusChanged || eventStatusLocal.extensionDataChanged) {
                        ...
                        if( eventDataUpdated ) {
                            ...
                            notifyEventDataChanged(eventParam);/* @req DEM475 */
                        }
                    }
                }
            } else {
                
                returnCode = E_NOT_OK; // Operation cycle not started
            }
        } else {
            
            returnCode = E_NOT_OK; // Operation cycle not set
        }
    } else {
        returnCode = E_NOT_OK;// Event ID not configured
    }
...
static void getEventStatus(Dem_EventIdType eventId, Dem_EventStatusExtendedType *eventStatusExtended)
{
    ...
    if (eventStatusLocal.eventId == eventId) {
        ...
        *eventStatusExtended =DEM_TEST_NOT_COMPLETED_SINCE_LAST_CLEAR;
		//20160709,xy,for fe-7
	}
}
 ...
 */
static Std_ReturnType getFaultDetectionCounter(Dem_EventIdType eventId, sint8_t *counter)
{
    ...
    if((evtStatusBuffer->eventStatusExtended & DEM_CONFIRMED_DTC) &&
      ...
      // &&(!(evtStatusBuffer->eventStatusExtended & DEM_TEST_NOT_COMPLETED_THIS_OPERATION_CYCLE))
        ...
        if (agingRecFound) {
            ...
            /* IMPROVEMENT: Should it be >= instead of > in the expression below? */
            ...
            updatedMemory = TRUE;
			osal_nv_write(DTC_NV_AGE(evtStatusBuffer->eventId),  sizeof(HealingRecType), agingRecLocal );
...
static Std_ReturnType handleAging(Dem_OperationCycleIdType operationCycleId, Dem_OperationCycleStateType cycleState)
{
    ...
    if( agingUpdatedPrimaryMemory ) {
    	//setAgingBlockChanged(DEM_DTC_ORIGIN_PRIMARY_MEMORY); Xiaojun modify
    }
    if( agingUpdatedSecondaryMemory ) {
        //setAgingBlockChanged(DEM_DTC_ORIGIN_SECONDARY_MEMORY); Xiaojun modify
    }
...
static Std_ReturnType setOperationCycleState(Dem_OperationCycleIdType operationCycleId, Dem_OperationCycleStateType cycleState) /** @req DEM338 */
{
    ...
    if (operationCycleId < DEM_OPERATION_CYCLE_ID_ENDMARK) {
        ...
        case DEM_CYCLE_STATE_START:
            ...
            for (i = 0; i < DEM_MAX_NUMBER_EVENT; i++) {// Lookup event ID
                if( eventStatusBuffer[i].eventId != DEM_EVENT_ID_NULL ) {
                    if( eventStatusBuffer[i].eventParamRef->EventClass->OperationCycleRef == operationCycleId ) {
                    	sint8_t startFDC = getDefaultUDSFdc(eventStatusBuffer[i].eventId);
						...
						//20160709,xy,for fe-7
                        //eventStatusBuffer[i].eventStatusExtended |= (Dem_EventStatusExtendedType)DEM_TEST_NOT_COMPLETED_THIS_OPERATION_CYCLE;
						//20160709,xy,for fe-7
...
static inline void initEventStatusBuffer(const Dem_EventParameterType *eventIdParamList)
{
	...
	for (i = 0; i < DEM_MAX_NUMBER_EVENT; i++) {
		eventStatusRecPtr = &eventStatusBuffer[i];
        if(NULL != eventStatusRecPtr) {
        	sint8_t startUdsFdc = getDefaultUDSFdc(eventIdParamList[i].EventID);
            eventStatusRecPtr->eventId 			= eventIdParamList[i].EventID;
            eventStatusRecPtr->eventParamRef 	= &eventIdParamList[i];
...
#ifdef DEM_USE_MEMORY_FUNCTIONS
#if !defined(CFG_DEM_AGING_PROCESSING_EXTENSION)
static boolean validateAgingBuffer(HealingRecType* agingBuffer, uint32_t agingBufferSize, Dem_DTCOriginType origin)
{
...
static boolean validateFreezeFrames(FreezeFrameRecType* freezeFrameBuffer, uint32_t freezeFrameBufferSize, Dem_DTCOriginType origin)
{
    ...
    for (i = 0; i < freezeFrameBufferSize; i++) {
...
static boolean validateExtendedData(ExtDataRecType* extendedDataBuffer, uint32_t extendedDataBufferSize, Dem_DTCOriginType origin)
{
    boolean extendedDataBlockChanged = FALSE;
    uint16_t i;
    
    for (i = 0; i < extendedDataBufferSize; i++) {
...
#ifdef DEM_USE_MEMORY_FUNCTIONS
static boolean ValidateAndMergeEventRecords(EventRecType* eventBuffer, uint32_t eventBufferSize, boolean* eventEntryChanged,
                                    uint32_t* Evt_TimeStamp, Dem_DTCOriginType origin ) {
...
static void MergeBuffer(Dem_DTCOriginType origin) {
    ...
    uint32_t 				extendedDataBufferSize = 0;
...
//    Irq_Save(state);
    ...
    for(i = 0; i < DEM_MAX_NUMBER_EVENT; i++) {/* Notify application if event data was updated */
        if( eventStatusBuffer[i].errorStatusChanged ) {/* @req DEM475 */
            notifyEventDataChanged(eventStatusBuffer[i].eventParamRef);
            eventStatusBuffer[i].errorStatusChanged = FALSE;
        }
    }
	...
	for (i=0;i<DEM_MAX_NUMBER_EVENT;i++) {
		event_id = (configSet->EventParameter)[i].EventID;
		if(0 == osal_nv_item_init( DTC_NV_EVENT(event_id), sizeof(EventStatusRecType), &eventStatusBuffer[i]))
			osal_nv_read( DTC_NV_EVENT(event_id),  sizeof(EventStatusRecType), &eventStatusBuffer[i]);
		eventStatusRecPtr = &eventStatusBuffer[i];
		eventStatusRecPtr->eventId = event_id;
    	eventStatusRecPtr->eventParamRef 	= &eventIdParamList[i];
	}
	for (i=0;i<DEM_MAX_NUMBER_EVENT;i++) {
		event_id = (configSet->EventParameter)[i].EventID ;
		if(0 == osal_nv_item_init( DTC_NV_FREEZE(event_id), sizeof(FreezeFrameRecType), &priMemFreezeFrameBuffer[i]))
			osal_nv_read( DTC_NV_FREEZE(event_id),  sizeof(FreezeFrameRecType), &priMemFreezeFrameBuffer[i]);
	}
	for (i=0;i<DEM_MAX_NUMBER_EVENT;i++) {
		event_id = (configSet->EventParameter)[i].EventID;
		if(0 == osal_nv_item_init( DTC_NV_EXT(event_id), sizeof(ExtDataRecType), &priMemExtDataBuffer[i]))
			osal_nv_read( DTC_NV_EXT(event_id),  sizeof(ExtDataRecType), &priMemExtDataBuffer[i]);
	}
	
	for (i=0;i<DEM_MAX_NUMBER_EVENT;i++) {
		priMemAgingBuffer[i].eventId 		= 0;
		priMemAgingBuffer[i].agingCounter 	= 0;
		event_id = (configSet->EventParameter)[i].EventID;
		if(0 == osal_nv_item_init( DTC_NV_AGE(event_id), sizeof(HealingRecType), &priMemAgingBuffer[i]))
			osal_nv_read( DTC_NV_AGE(event_id),  sizeof(HealingRecType), &priMemAgingBuffer[i]);
	}
...
#if 0 //dengwei    
		return ilog2((uint64_t)(0xFFFFFFFFFFFFFFFFul & val));
#else
		//return 0x01;
#endif
...
void Dem_Shutdown(void)
{
	...
	union {
		FreezeFrameRecType frame;
		ExtDataRecType extend;
		//EventStatusRecType event;
	} frame_extend;

	while (flash_store_event & 0x3FFFFFFFFE) {
       	eventid = ilog2_64(flash_store_event & 0x3FFFFFFFFE);   /* Find mailbox */
       	lookupEventStatusRec(eventid, &eventStatusRecPtr);
		//osal_nv_read( DTC_NV_EVNET(eventid),  sizeof(EventStatusRecType), &frame_extend.event);
		//if (memcmp(&frame_extend.event,eventStatusRecPtr ,sizeof(EventStatusRecType))) {
		    osal_nv_write( DTC_NV_EVENT(eventid),  sizeof(EventStatusRecType), (void *)eventStatusRecPtr );
			
		//}
	   flash_store_event ^= 1 << eventid;
	}

	for (i=0;i<DEM_MAX_NUMBER_EVENT;i++) {
		osal_nv_read( DTC_NV_FREEZE(i+1),  sizeof(FreezeFrameRecType), &frame_extend.frame);
		if (memcmp(&frame_extend.frame,&priMemFreezeFrameBuffer[i],sizeof(FreezeFrameRecType))) {
			osal_nv_write(DTC_NV_FREEZE(i+1),  sizeof(FreezeFrameRecType), &priMemFreezeFrameBuffer[i] );
		}
	}
	for (i=0;i<DEM_MAX_NUMBER_EVENT;i++) {
		osal_nv_read( DTC_NV_EXT(i+1),  sizeof(ExtDataRecType), &frame_extend.extend);
		if (memcmp(&frame_extend.extend,&priMemExtDataBuffer[i],sizeof(ExtDataRecType))) {
			osal_nv_write(DTC_NV_EXT(i+1),  sizeof(ExtDataRecType), &priMemExtDataBuffer[i] );
		}
	}
...
unsigned char temp;
temp = (unsigned char)eventStatus;

    VALIDATE_RV(DEM_INITIALIZED == demState, DEM_SETEVENTSTATUS_ID, DEM_E_UNINIT, E_NOT_OK);
	VALIDATE_RV(IS_VALID_EVENT_STATUS((Dem_EventStatusType)(temp)), DEM_SETEVENTSTATUS_ID, DEM_E_PARAM_DATA, E_NOT_OK);
...
//    returnCode = handleEvent(eventId, eventStatus);
	returnCode = handleEvent(eventId, (Dem_EventStatusType)(temp));
 ...
 * Reentrant:   No
 */
Std_ReturnType Dem_GetFaultDetectionCounter(Dem_EventIdType eventId, sint8_t *counter)
{
    Std_ReturnType returnCode = E_OK;
//    imask_t state;
//    Irq_Save(state);
 ...
 * Reentrant:   Yes
 */
Std_ReturnType Dem_GetDTCOfEvent(Dem_EventIdType eventId, Dem_DTCFormatType dtcFormat, uint32_t* dtcOfEvent)
{
...
//    Irq_Save(state);
    ...
    if (demState == DEM_INITIALIZED) // No action is taken if the module is not started
    {
        ...
        if (eventParam != NULL) {
            // IMPROVEMENT: DTCFormat issue (UDS, OBD)
            if (eventParam->DTCClassRef != NULL) {
 ...
 * Reentrant:   No
 */
Std_ReturnType Dem_GetDTCStatusAvailabilityMask(uint8_t *dtcStatusMask) /** @req DEM014 */
{
    /** @req DEM060 */
 ...
 * Reentrant:   No
 */
Dem_ReturnSetFilterType Dem_SetDTCFilter(uint8_t dtcStatusMask,
        Dem_DTCKindType dtcKind,
        Dem_DTCFormatType dtcFormat,
        Dem_DTCOriginType dtcOrigin,
        Dem_FilterWithSeverityType filterWithSeverity,
 ...
 * Reentrant:   No
 */
Dem_ReturnGetStatusOfDTCType Dem_GetStatusOfDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, Dem_EventStatusExtendedType* status) {
 ...
 * Reentrant:   No
 */
Dem_ReturnGetNumberOfFilteredDTCType Dem_GetNumberOfFilteredDtc(uint16_t *numberOfFilteredDTC) {
    uint16_t i;
    uint16_t numberOfFaults = 0;
 ...
 * Reentrant:   No
 */
Dem_ReturnGetNextFilteredDTCType Dem_GetNextFilteredDTC(uint32_t *dtc, Dem_EventStatusExtendedType *dtcStatus)
{
...
//    Irq_Save(state);
    Disable_Interrupts();
    if (demState == DEM_INITIALIZED) {
        // NOTE: This job should be done in an more advanced way according to Dem217
            ...
            if (eventStatusBuffer[dtcFilter.faultIndex].eventId != DEM_EVENT_ID_NULL) {
                if (matchEventWithDtcFilter(&eventStatusBuffer[dtcFilter.faultIndex])) {
                    if ((eventStatusBuffer[dtcFilter.faultIndex].eventParamRef->DTCClassRef != NULL)
						&&(eventStatusBuffer[dtcFilter.faultIndex].eventParamRef->Arc_EOL != TRUE)) {
 ...
 * Reentrant:   No
 */
Dem_ReturnClearDTCType Dem_ClearDTC(bool flash,uint32_t dtc, Dem_DTCFormatType dtcFormat, Dem_DTCOriginType dtcOrigin) /** @req DEM009 *//** @req DEM241 */
{
...
//    Irq_Save(state);
    ...
    if (demState == DEM_INITIALIZED) {
        for (i = 0; i < DEM_MAX_NUMBER_EVENT; i++) {
            dataDeleted = FALSE;
			eventId = (configSet->EventParameter)[i].EventID;
            eventStatusBuffer[i].eventId = eventId;
			eventStatusBuffer[i].eventParamRef 	= &eventIdParamList[i];
                ...
                if (eventParam != NULL) {
                    ...
                    if ((DEM_CLEAR_ALL_EVENTS == STD_ON) || (eventParam->DTCClassRef != NULL)) {
                        if (checkDtcGroup(dtc, eventParam)) {
							...
							if(FALSE == Vehicle_at_standstill_flag)//vehiclespeed < 3mil
							{
							     Enable_Interrupts();
								return DEM_CLEAR_LIMIT;
							}
                            ...
                            if( eventParam->EventClass->EventDestination == dtcOrigin ) {
                                ...
                                if( clearAllowed ) {
                                    ...
                                    case DEM_DTC_ORIGIN_PRIMARY_MEMORY:
                                        /** @req DEM077 */
                                        if( deleteEventMem(flash,eventParam, priMemEventBuffer, DEM_MAX_NUMBER_EVENT_PRI_MEM, DEM_DTC_ORIGIN_PRIMARY_MEMORY) ) {
                                            dataDeleted = TRUE;
                                        }
                                        if( deleteFreezeFrameDataMem(flash,eventParam, DEM_DTC_ORIGIN_PRIMARY_MEMORY) ) {
                                            dataDeleted = TRUE;
                                        }
                                        if( deleteExtendedDataMem(flash,eventParam, DEM_DTC_ORIGIN_PRIMARY_MEMORY) ) {
                                            dataDeleted = TRUE;
                                        }
...
Dem_ReturnControlDTCStorageType Dem_EnableDTCSetting(Dem_DTCGroupType dtcGroup, Dem_DTCKindType dtcKind)
{
    ...
    if (demState == DEM_INITIALIZED) {
		...
		disableDtcSetting.dtcGroup = 0;
        disableDtcSetting.dtcKind = 0xf;
    } else {
 ...
 * Reentrant:   No
 */
Dem_ReturnGetExtendedDataRecordByDTCType Dem_GetExtendedDataRecordByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t extendedDataNumber, uint8_t *destBuffer, uint16_t *bufSize)
{
 ...
 * Reentrant:   No
 */
Dem_ReturnGetSizeOfExtendedDataRecordByDTCType Dem_GetSizeOfExtendedDataRecordByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t extendedDataNumber, uint16_t *sizeOfExtendedDataRecord)
{
    Dem_ReturnGetExtendedDataRecordByDTCType returnCode = DEM_GET_SIZEOFEDRBYDTC_W_DTC;
    EventStatusRecType *eventRec;
    Dem_ExtendedDataRecordClassType const *extendedDataRecordClass = NULL;
    uint16_t posInExtData;
 ...
 */
/** @req DEM236 */
Dem_ReturnGetFreezeFrameDataByDTCType Dem_GetFreezeFrameDataByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t recordNumber, uint8_t* destBuffer, uint16_t*  bufSize)
{
 ...
 */
 /** @req DEM238 */
Dem_ReturnGetSizeOfFreezeFrameType Dem_GetSizeOfFreezeFrameByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t recordNumber, uint16_t* sizeOfFreezeFrame)
{
...
Dem_ReturnSetFilterType Dem_SetFreezeFrameRecordFilter(Dem_DTCFormatType DTCFormat, uint16_t *NumberOfFilteredRecords)
{
...
Dem_ReturnGetNextFilteredDTCType Dem_GetNextFilteredRecord(uint32_t *DTC, uint8_t *RecordNumber)
{
...
//    Irq_Save(state);
    ...
    if (demState == DEM_INITIALIZED) {
        ...
        boolean found = FALSE;
        for(i = ffRecordFilter.ffIndex; (i < DEM_MAX_NUMBER_FF_DATA_PRI_MEM) && !found; i++  ) {
            if( DEM_EVENT_ID_NULL != priMemFreezeFrameBuffer[i].eventId ) {
                lookupEventStatusRec(priMemFreezeFrameBuffer[i].eventId, &eventStatusRecPtr);
               if( (NULL != eventStatusRecPtr) && (NULL != eventStatusRecPtr->eventParamRef->DTCClassRef)) {
                   /* Found one! */
 ...
 */
 /* @req OBD_DEM_REQ_3 */
Std_ReturnType Dem_GetDTCOfOBDFreezeFrame(uint8_t FrameNumber, uint32_t* DTC )
{
 ...
 */
 /* @req OBD_DEM_REQ_2 */
Std_ReturnType Dem_GetDTCOfOBDFreezeFrame(uint8_t FrameNumber, uint32_t* DTC );
Std_ReturnType Dem_GetOBDFreezeFrameData(uint8_t PID1, uint8_t DataElementIndexOfPid, uint8_t* DestBuffer, uint8_t* BufSize)
            ...
            if(freezeFrameClass->FFKind == DEM_FREEZE_FRAME_OBD){
                if(freezeFrameClass->FFIdClassRef != NULL){
                    for(i = 0; (i < DEM_MAX_NR_OF_PIDS_IN_FREEZEFRAME_DATA) && ((freezeFrameClass->FFIdClassRef[i]->Arc_EOL) == FALSE); i++){
                        ...
                        if(freezeFrameClass->FFIdClassRef[i]->PidIdentifier == PID1){
...
#if 0/*del by mgd on 20210715*/
        if((pidFound == TRUE) && (NULL != freezeFrame) && (offset >= DEM_PID_IDENTIFIER_SIZE_OF_BYTES)){
            if(((*BufSize) >= pidDataSize) && (PID1 == (freezeFrame->data[offset - DEM_PID_IDENTIFIER_SIZE_OF_BYTES])) \
    ...
    for (i = 0; ((i < DEM_MAX_NR_OF_PIDS_IN_FREEZEFRAME_DATA) && (!FFIdClassRef[i]->Arc_EOL)); i++) {
        if(FFIdClassRef[i]->PidOrDidUsePort == FALSE){
            ...
            if ((storeIndex + recordSize + DEM_PID_IDENTIFIER_SIZE_OF_BYTES) <= DEM_MAX_SIZE_FF_DATA) {
                /* store PID */
                (*freezeFrame)->data.data[storeIndex] = FFIdClassRef[i]->PidIdentifier;/*modify "(*freezeFrame)->data" to "(*freezeFrame)->data.data" by mgd on 20210715*/
                storeIndex++;
                /* store data */
                if(FFIdClassRef[i]->PidReadFnc != NULL){
                    callbackReturnCode = FFIdClassRef[i]->PidReadFnc(&(*freezeFrame)->data.data[storeIndex]);/*modify "(*freezeFrame)->data" to "(*freezeFrame)->data.data" by mgd on 20210715*/
                    if (callbackReturnCode != E_OK) {
                        memset(&(*freezeFrame)->data.data[storeIndex], DEM_FREEZEFRAME_DEFAULT_VALUE, recordSize);/*modify "(*freezeFrame)->data" to "(*freezeFrame)->data.data" by mgd on 20210715*/
                    }
                    storeIndex += recordSize;
            ...
            //NOTE: The RTE should provide the port
 ...
 static void getDidData(const Dem_PidOrDidType **const *didClassPtr, FreezeFrameRecType **freezeFrame, uint16_t *storeIndexPtr)
{
    ...
    for (i = 0; ((i < DEM_MAX_NR_OF_DIDS_IN_FREEZEFRAME_DATA) && (!FFIdClassRef[i]->Arc_EOL)); i++) {
        if(FFIdClassRef[i]->PidOrDidUsePort == FALSE){
            ...
            }else{/* if no readDidDataLengthFunction, then try the PidOrDidSize. */
                recordSize = FFIdClassRef[i]->PidOrDidSize;
            }
            ...
            //IMPROVEMENT : RTE should provide the port
            DET_REPORTERROR(MODULE_ID_DEM, 0, DEM_GET_FREEZEFRAME_ID, DEM_DSP_DID_USE_PORT_IS_TRUE);
            break;
        }
    }
    ...
    //store storeIndex,it will be used for judge whether FF contains valid data.
    *storeIndexPtr = storeIndex;
	#else/*modify by mgd on 20210715*/
	   (*freezeFrame)->data.Snapshot.ECUVoltage = veh_data.KL30_Vol;
	   (*freezeFrame)->data.Snapshot.VehicleSpeed[0] = (veh_data.VehicleSpeed>>8)&0xff;
	   (*freezeFrame)->data.Snapshot.VehicleSpeed[1] = veh_data.VehicleSpeed&0xff;
	   if((*freezeFrame)->data.Snapshot.OccurrenceCounter<0xff)
	   {
	      (*freezeFrame)->data.Snapshot.OccurrenceCounter++;
	   }
	   if( 1 == (*freezeFrame)->data.Snapshot.OccurrenceCounter )
	   {
	     (*freezeFrame)->data.Snapshot.TheOdometerOfFirstMalfunction[0] = (veh_data.IPKTotalOdometer>>16)&0xff;
		 (*freezeFrame)->data.Snapshot.TheOdometerOfFirstMalfunction[1] = (veh_data.IPKTotalOdometer>>8)&0xff;
		 (*freezeFrame)->data.Snapshot.TheOdometerOfFirstMalfunction[2] = (veh_data.IPKTotalOdometer>>0)&0xff;
	   }
	   (*freezeFrame)->data.Snapshot.TheOdometerOfLastMalfunction[0] = (veh_data.IPKTotalOdometer>>16)&0xff;
	   (*freezeFrame)->data.Snapshot.TheOdometerOfLastMalfunction[1] = (veh_data.IPKTotalOdometer>>8)&0xff;
	   (*freezeFrame)->data.Snapshot.TheOdometerOfLastMalfunction[2] = (veh_data.IPKTotalOdometer>>0)&0xff;
	   *storeIndexPtr = 10;
	#endif
}
