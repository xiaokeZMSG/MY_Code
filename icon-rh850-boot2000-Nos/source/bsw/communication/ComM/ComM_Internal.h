...
#define COMM_DET_REPORTERROR(serviceId, errorId)			\
	(void)Det_ReportError(MODULE_ID_COMM, 0, serviceId, errorId)
...
typedef struct {
	...
	uint32_t						UserRequestMask;
	ComM_InhibitionStatusType	InhibitionStatus;
	uint32_t						FullComMinDurationTimeLeft;
	uint32_t						LightTimeoutTimeLeft;
	uint8_t						NmIndicationMask;
...
typedef struct {
	...
	uint16_t						InhibitCounter;  /**< @req COMM138  @req COMM141 */
} ComM_InternalType;

#define COMM_NM_INDICATION_NONE					(uint8_t)(0u)
#define COMM_NM_INDICATION_NETWORK_MODE			(uint8_t)(1u)
#define COMM_NM_INDICATION_PREPARE_BUS_SLEEP	(uint8_t)(1u << 1)
#define COMM_NM_INDICATION_BUS_SLEEP			(uint8_t)(1u << 2)
#define COMM_NM_INDICATION_RESTART				(uint8_t)(1u << 3)
