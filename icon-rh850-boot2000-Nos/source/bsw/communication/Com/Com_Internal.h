...
extern Com_BufferPduStateType Com_BufferPduState[COM_MAX_N_IPDUS];
...
#define DET_REPORTERROR(_api, _error)                   \
    do {                                                  \
        (void)Det_ReportError(MODULE_ID_COM, 0, _api, _error); \
    } while(0)
#else
...
#define TESTBIT(source,bit)	( *( (uint8_t *)source  + (bit / 8) ) &  (uint8_t)(1u << (bit % 8)) )
#define SETBIT(dest,bit)	( *( (uint8_t *)dest    + (bit / 8) ) |= (uint8_t)(1u << (bit % 8)) )
#define CLEARBIT(dest,bit)	( *( (uint8_t *)dest    + (bit / 8) ) &= (uint8_t)~(uint8_t)(1u << (bit % 8)) )
