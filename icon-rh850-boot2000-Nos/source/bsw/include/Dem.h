...
extern uint8_t secAcssAttempts[4];
...
Std_ReturnType Dem_GetFaultDetectionCounter(Dem_EventIdType eventId, int8_t *counter); /** @req DEM203 */
...
Std_ReturnType Dem_GetDTCOfEvent(Dem_EventIdType eventId, Dem_DTCFormatType dtcFormat, uint32_t* dtcOfEvent); /** @req DEM198 */
#if (DEM_ENABLE_CONDITION_SUPPORT == STD_ON)
Std_ReturnType Dem_SetEnableCondition(uint8_t EnableConditionID, boolean ConditionFulfilled); /** @req DEM201*/
#endif
...
Dem_ReturnSetFilterType Dem_SetDTCFilter(uint8_t dtcStatusMask, Dem_DTCKindType dtcKind, Dem_DTCFormatType dtcFormat, Dem_DTCOriginType dtcOrigin, Dem_FilterWithSeverityType filterWithSeverity, Dem_DTCSeverityType dtcSeverityMask, Dem_FilterForFDCType filterForFaultDetectionCounter);  /** @req DEM208 */
Dem_ReturnGetStatusOfDTCType Dem_GetStatusOfDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, Dem_EventStatusExtendedType* status); /** @req DEM212 */
Std_ReturnType Dem_GetDTCStatusAvailabilityMask(uint8_t *dtcStatusMask); /** @req DEM213 */
Dem_ReturnGetNumberOfFilteredDTCType Dem_GetNumberOfFilteredDtc(uint16_t* numberOfFilteredDTC); /** @req DEM214 */
Dem_ReturnGetNextFilteredDTCType Dem_GetNextFilteredDTC(uint32_t* dtc, Dem_EventStatusExtendedType* dtcStatus); /** @req DEM215 */
...
Dem_ReturnGetFreezeFrameDataByDTCType Dem_GetFreezeFrameDataByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin,uint8_t recordNumber, uint8_t* destBuffer, uint16_t* bufSize);/** @req DEM236 */
Dem_ReturnGetSizeOfFreezeFrameType Dem_GetSizeOfFreezeFrameByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t recordNumber, uint16_t* sizeOfFreezeFrame);/** @req DEM238 */
Dem_ReturnGetExtendedDataRecordByDTCType Dem_GetExtendedDataRecordByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t extendedDataNumber, uint8_t *destBuffer, uint16_t *bufSize); /** @req DEM239 */
Dem_ReturnGetSizeOfExtendedDataRecordByDTCType Dem_GetSizeOfExtendedDataRecordByDTC(uint32_t dtc, Dem_DTCOriginType dtcOrigin, uint8_t extendedDataNumber, uint16_t *sizeOfExtendedDataRecord); /** @req DEM240 */
Dem_ReturnSetFilterType Dem_SetFreezeFrameRecordFilter(Dem_DTCFormatType dtcFormat, uint16_t *NumberOfFilteredRecords);/* @req DEM209 */
Dem_ReturnGetNextFilteredDTCType Dem_GetNextFilteredRecord(uint32_t *DTC, uint8_t *RecordNumber); /* @req DEM224 */

/* DTC storage */
//Dem_ReturnClearDTCType Dem_ClearDTC(DTC_Clear_Type clear_type,uint32 dtc, Dem_DTCFormatType dtcFormat, Dem_DTCOriginType dtcOrigin); /** @req DEM241 */
Dem_ReturnClearDTCType Dem_ClearDTC(bool flash,uint32_t dtc, Dem_DTCFormatType dtcFormat, Dem_DTCOriginType dtcOrigin); /** @req DEM241 */
 ...
 *
 */
//Std_ReturnType Dem_GetOBDFreezeFrameData(uint8_t PID, uint8_t DataElementIndexOfPid, uint8_t * DestBuffer, uint8_t * BufSize);
...
void Dem_CallbackEventStatusChangedFncWithId(Dem_EventIdType EventId, Dem_EventStatusExtendedType EventStatusOld, Dem_EventStatusExtendedType EventStatusNew); /** @req DEM259 */

extern boolean DemEnableConditions[DEM_NUM_ENABLECONDITIONS];
