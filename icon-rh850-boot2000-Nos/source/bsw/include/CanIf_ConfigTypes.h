...
typedef struct {
	...
	uint8_t CanIfBufferSize;
	uint8_t CanIf_Arc_BufferId;
...
typedef void (*CanIfBusOffNotificationType)(uint8_t Controller);
typedef void (*CanIfWakeUpNotificationType)();
typedef void (*CanIfWakeupValidNotificationType)();
typedef void (*CanIfControllerModeIndicationType)(uint8_t ControllerId, CanIf_ControllerModeType ControllerMode);
typedef void (*CanIfErrorNotificatonType)(uint8_t,Can_Arc_ErrorType);
...
typedef struct {
	...
	uint32_t CanIfUserTxConfirmation;
...
typedef struct {
   ...
   *  Extended CAN identifier */
	uint32_t 		CanIfCanRxPduLowerCanId;
	uint32_t 		CanIfCanRxPduUpperCanId;
...
//	CanIf_UserTypeType  CanIfRxUserType;
   ...
   *  upper layers (PduRouter, CanNm, CanTp and ComplexDeviceDrivers). If parameter
   *  is NO_FUNCTION_CALLOUT no call-out function is configured. */
	uint32_t CanIfUserRxIndication;
...
//	CanIf_SoftwareFilterTypeType CanIfSoftwareFilterType;
   ...
   *  ArcCore exension */
	uint32_t CanIfCanRxPduCanIdMask;
...
typedef struct {
	...
	void (*CanIfBusOffNotification)(uint8_t Controller);
	 ...
	 *  configured. */
	void (*CanIfControllerModeIndication)(uint8_t ControllerId, CanIf_ControllerModeType ControllerMode);

	/** ArcCore ext. */
	void (*CanIfErrorNotificaton)(uint8_t,Can_Arc_ErrorType);
...
typedef struct {
	...
	/** Not used */
	uint32_t CanIfNumberOfDynamicCanTXPduIds;

	uint16_t CanIfNumberOfTxBuffers;
