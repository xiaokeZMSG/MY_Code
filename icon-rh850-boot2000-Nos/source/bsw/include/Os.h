...
/* IMPROVEMENT: OSMEMORY_IS__ , see 8.2*/
...
typedef uint16_t ScheduleTableType;
typedef uint16_t GlobalTimeTickType;
...
typedef uint16_t TrustedFunctionIndexType;
typedef  void * TrustedFunctionParameterRefType;

/* See 8.3.9 */
#define INVALID_ISR		((sint16)(-1))
typedef	int16_t ISRType;

#define APP_NO_OWNER	(-1UL)

typedef void * MemoryStartAddressType;
typedef uint32_t MemorySizeType;
...
extern StatusType Os_SysCall(uint32_t index,...);
extern StatusType Os_SysCall_0(uint32_t index);
...
//#define Os_EnterSupervisorMode()        SYS_CALL(SYS_CALL_EnterSupervisorMode);
 ...
 * System timer
 *-----------------------------------------------------------------*/
typedef const uint32_t OsTickType;
