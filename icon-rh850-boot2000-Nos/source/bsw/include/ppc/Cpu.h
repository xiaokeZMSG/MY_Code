...
#define isync()  		asm volatile(" se_isync");
#define msync()  		asm volatile(" se_isync");
#else
 ...
 * @return
 */
static inline unsigned char ilog2( unsigned int val ) {
...
#if defined(__DCC__)
asm volatile void _Irq_Disable_restore(unsigned long flags)
{
% reg flags
  wrtee flags
}
