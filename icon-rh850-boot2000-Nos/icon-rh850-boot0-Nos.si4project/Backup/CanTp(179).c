...
#define VALIDATE(_exp,_api,_err ) \
		if( !(_exp) ) { \
			(void)Det_ReportError(MODULE_ID_CANTP, 0, _api, _err); \
			return E_NOT_OK; \
		}
...
#define DET_REPORTERROR(_api, _error)                     \
    do {                                                  \
        (void)Det_ReportError(MODULE_ID_CANTP, 0, _api, _error); \
    } while(0)

#else
#define VALIDATE(_exp,_api,_err )
#define VALIDATE_NO_RV(_exp,_api,_err )
#define DET_REPORTERROR(_exp,_api,_err)
#endif
...
#define TIMER_DECREMENT(timer) \
        if (timer >= CanTp_ConfigPtr->CanTpGeneral->main_function_period) { \
            timer = timer - CanTp_ConfigPtr->CanTpGeneral->main_function_period; \
        }else { \
        	timer = 0; \
        }
...
typedef enum {
	UNINITIALIZED, 
	IDLE, 
	SF_OR_FF_RECEIVED_WAITING_PDUR_BUFFER, /** @req CANTP079 */
...
typedef struct {
	uint8_t data[MAX_SEGMENT_DATA_SIZE];
 ...
 */
typedef struct {
	uint16_t nextFlowControlCount; // Count down to next Flow Control.
	uint16_t framesHandledCount; // Counter keeping track total frames handled.
	uint32_t stateTimeoutCount; // Counter for timeout.
	uint8_t extendedAddress; // Not always used but need to be available.
	uint8_t STmin; // In case we are transmitters the remote node can configure this value (only valid for TX).
	uint8_t BS; // Blocksize (only valid for TX).
	boolean NasNarPending;
	uint32_t NasNarTimeoutCount; // CanTpNas, CanTpNar.
...
static uint32_t ConvertMsToMainCycles(uint32_t ms) {
...
ISO15765FrameType getFrameType(
	...
	case ISO15765_TPCI_SF:
		if(tpci<=ISO15765_TPCI_DL)//add:xy 20160526,single frame max lenght is 0x07,if tpci is 0x08 or 0x09,will be return a negative response
			res = SINGLE_FRAME;
		break;
	...
	case ISO15765_TPCI_FC: // Some kind of flow control.
		...
		default :
			...
			res = INVALID_FRAME;//res = FLOW_CONTROL_CTS_FRAME;   //change,20160617,issue TP27
			break;
		}
	}
	return res;
...
BufReq_ReturnType copySegmentToPduRRxBuffer(const CanTp_RxNSduType *rxConfig,
		CanTp_SimplexChannelPrivateType *rxRuntime, uint8_t *segment,
	...
	*bytesWrittenSuccessfully = 0;
	...
	{
		ret = Dcm_StartOfReception(rxConfig->PduR_PduId, rxRuntime->transferTotal, &rxRuntime->sizeBuffer); /** @req CANTP079 */
	...
	else
	{
		...
		tempPdu.SduLength = 0;
		tempPdu.SduDataPtr = NULL;
		ret = Dcm_CopyRxData(rxConfig->PduR_PduId, &tempPdu, &rxRuntime->sizeBuffer);
	}
...
Std_ReturnType canReceivePaddingHelper(
	...
	return CanIf_Transmit(rxConfig->CanIf_FcPduId+CANIF_TXPDU_ID_0x738, PduInfoPtr);
}
...
void sendFlowControlFrame(const CanTp_RxNSduType *rxConfig, CanTp_SimplexChannelPrivateType *rxRuntime, BufReq_ReturnType flowStatus) {
	...
	PduInfoType pduInfo;
	uint8_t sduData[8]; // Note that buffer in declared on the stack.
	uint16_t computedBs = 0;
	...
	case BUFREQ_OK:
	{
		...
		sduData[indexCount++] = (uint8_t) rxConfig->CanTpSTmin;
	...
	if (ret != E_OK) {
		Dcm_TpRxIndication(rxConfig->PduR_PduId, NTFRSLT_E_NOT_OK);  /** @req CANTP084 */
...
void handleConsecutiveFrame(const CanTp_RxNSduType *rxConfig,
	...
	if (rxRuntime->iso15765.state == RX_WAIT_CONSECUTIVE_FRAME) {
		...
		if (segmentNumber != (rxRuntime->iso15765.framesHandledCount & SEGMENT_NUMBER_MASK)) { /** @req CANTP314 */
			Dcm_TpRxIndication(rxConfig->PduR_PduId, NTFRSLT_E_WRONG_SN); /** @req CANTP084 */
			rxRuntime->iso15765.state = IDLE;
			rxRuntime->pduId = INVALID_PDU_ID;
			rxRuntime->mode = CANTP_RX_WAIT;
		} else {
			...
			if (ret == BUFREQ_E_NOT_OK) {
				...
				if (bytesLeftToTransfer > 0) {
					...
					rxRuntime->pduId = INVALID_PDU_ID;
					rxRuntime->mode = CANTP_RX_WAIT;
					Dcm_TpRxIndication(rxConfig->PduR_PduId, NTFRSLT_OK); /** @req CANTP084 */
				}
			}
		}
		}
...
BufReq_ReturnType sendNextTxFrame(
	...
	uint8_t offset = txRuntime->canFrameBuffer.byteCount;
	...
	ret = Dcm_CopyTxData(txConfig->PduR_PduId, &pduInfo, NULL, &txRuntime->sizeBuffer); /** @req CANTP272 */ /** @req CANTP226 */ /** @req CANTP086 */
...
void handleNextTxFrameSent(
	...
	if (txRuntime->transferTotal <= txRuntime->transferCount) {
		// Transfer finished!
		Dcm_TpTxConfirmation(txConfig->PduR_PduId, NTFRSLT_OK); /** @req CANTP090 *//** @req CANTP204 */
		...
		if (resp == BUFREQ_OK ) {
			...
			Dcm_TpTxConfirmation(txConfig->PduR_PduId, NTFRSLT_E_NOT_OK);  /** @req CANTP177 */ /** @req CANTP084 */ /** @req CANTP087 */
			txRuntime->iso15765.state = IDLE;
			txRuntime->pduId = INVALID_PDU_ID;
			txRuntime->mode = CANTP_TX_WAIT;
		}
		...
		txRuntime->iso15765.stateTimeoutCount = ConvertMsToMainCycles(txRuntime->iso15765.STmin);
	...
	if ( txRuntime->iso15765.state == TX_WAIT_FLOW_CONTROL )
	{
        switch (txPduData->SduDataPtr[indexCount++] & ISO15765_TPCI_FS_MASK)
		{
			...
			{	// This construction is added to make the hcs12 compiler happy.
				const uint8_t bs = txPduData->SduDataPtr[indexCount++];
			...
			case ISO15765_FLOW_CONTROL_STATUS_WAIT:
				...
				break;
			case ISO15765_FLOW_CONTROL_STATUS_OVFLW:
				Dcm_TpTxConfirmation(txConfig->PduR_PduId, NTFRSLT_E_NO_BUFFER); /* @req ISO/FDIS 15765-2:2004(E) 7.5.5.2*/ /** @req CANTP309 */
				txRuntime->iso15765.state = IDLE;
				txRuntime->pduId = INVALID_PDU_ID;
				txRuntime->mode = CANTP_TX_WAIT;
				break;
			...
			default:
				...
				Dcm_TpTxConfirmation(txConfig->PduR_PduId,NTFRSLT_E_INVALID_FS); /* @req ISO/FDIS 15765-2:2004(E) 7.5.5.3*/ /** @req CANTP317 */
				txRuntime->iso15765.state = IDLE;
				txRuntime->pduId = INVALID_PDU_ID;
				txRuntime->mode = CANTP_TX_WAIT;
				break;
...
void handleSingleFrame(const CanTp_RxNSduType *rxConfig,
	...
	if ((rxRuntime->iso15765.state != IDLE) && (rxRuntime->iso15765.state != RX_WAIT_CF_SDU_BUFFER) &&
		...
		pdurPduOngoing=CanTp_ConfigPtr->CanTpNSduList[rxRuntime->pduId].configData.CanTpRxNSdu.PduR_PduId;

		Dcm_TpRxIndication(pdurPduOngoing, NTFRSLT_E_NOT_OK); /** @req CANTP084 */ // Abort current reception, we need to tell the current receiver it has been aborted.
	}
	if (rxConfig->CanTpAddressingFormant == CANTP_EXTENDED)
	{
		uint8_t extendedAddress = 0;
		...
		const PduInfoType *CanTpTxInfoPtr)  /** @req CANTP176 */
{
	...
	txsdu_index = CanTpTxSduId + CanTp_ConfigPtr->CanTpGeneral->start_of_txsdu;
	if( CanTp_ConfigPtr->CanTpRxIdList[txsdu_index].CanTpNSduIndex != 0xFFFF ) {
		CanTp_InternalTxNSduId = CanTp_ConfigPtr->CanTpRxIdList[txsdu_index].CanTpNSduIndex;
		...
		if (txRuntime->iso15765.state == IDLE) {
			...
			if (txConfig->CanTpAddressingMode == CANTP_EXTENDED) { /** @req CANTP094 *//** @req CANTP095 */
				txRuntime->canFrameBuffer.data[txRuntime->canFrameBuffer.byteCount++] =
						(uint8_t) txConfig->CanTpNTa->CanTpNTa; // Target address.
			}
			switch(iso15765Frame) {
			case SINGLE_FRAME:
				txRuntime->canFrameBuffer.data[txRuntime->canFrameBuffer.byteCount++] =
						ISO15765_TPCI_SF | (uint8_t)(txRuntime->transferTotal);
				ret = E_OK;
				break;
			case FIRST_FRAME:
				txRuntime->canFrameBuffer.data[txRuntime->canFrameBuffer.byteCount++] =
						ISO15765_TPCI_FF | (uint8_t)((txRuntime->transferTotal & 0xf00) >> 8);
				txRuntime->canFrameBuffer.data[txRuntime->canFrameBuffer.byteCount++] =
						(uint8_t)(txRuntime->transferTotal & 0xff);
...
void CanTp_Init( const CanTp_ConfigType* CfgPtr)
{
	...
	for ( i=0; i < CANTP_MAX_NO_CHANNELS; i++) {
		...
		PduInfoType *CanTpRxPduPtr)
{
    ...
    CanTpRxPduId  -= CAN_DIAG_MSG_START;
	...
	if(( CANTP_ADD_FUNCTIONAL == rxConfigParams->CanTpRxTaType)&&(SINGLE_FRAME !=frameType))
	{//functinal addressing shall only be supported for single frame communication,20160508
		return;
	}
...
void CanTp_TxConfirmation(PduIdType CanTpTxPduId) /** @req CANTP076 */
{
	...
	/** @req CANTP236 */
	txsdu_index = CanTpTxPduId - CanTp_ConfigPtr->CanTpGeneral->start_of_txpdu + CanTp_ConfigPtr->CanTpGeneral->start_of_txsdu;
	if( CanTp_ConfigPtr->CanTpRxIdList[txsdu_index].CanTpNSduIndex != 0xFFFF ) {
		CanTpNSduId = CanTp_ConfigPtr->CanTpRxIdList[txsdu_index].CanTpNSduIndex;
		if ( CanTp_ConfigPtr->CanTpNSduList[CanTpNSduId].direction == IS015765_TRANSMIT ) {
			...
			txRuntime = &CanTpRunTimeData.runtimeDataList[txConfigParams->CanTpTxChannel].SimplexChnlList[CANTP_TX_CHANNEL];
...
void getBuffSingleFrame(const CanTp_RxNSduType *rxConfigListItem, CanTp_SimplexChannelPrivateType *rxRuntimeListItem) {
    ...
    if (ret == BUFREQ_OK)
    {
        Dcm_TpRxIndication(rxConfigListItem->PduR_PduId, NTFRSLT_OK); /** @req CANTP084 */
        /** @req CANTP204 */
    ...
    switch (txRuntimeListItem->iso15765.state) {
        ...
        case TX_WAIT_TRANSMIT: {
            ...
            if ( ret == BUFREQ_OK ) {
                ...
                if (txRuntimeListItem->iso15765.stateTimeoutCount == 0) {
                    ...
                    Dcm_TpTxConfirmation(txConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP204 */ /** @req CANTP280 */ /** @req CANTP185 */
                } 
                    } else {
                        txRuntimeListItem->iso15765.state = IDLE;
                        txRuntimeListItem->pduId = INVALID_PDU_ID;
                        txRuntimeListItem->mode = CANTP_TX_WAIT;
                        Dcm_TpTxConfirmation(txConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP204 */ /** @req CANTP087 */
                    }
                    break;
                }
        case TX_WAIT_FLOW_CONTROL:
            ...
            if (txRuntimeListItem->iso15765.stateTimeoutCount == 0) {
                ...
                txRuntimeListItem->pduId = INVALID_PDU_ID;
                txRuntimeListItem->mode = CANTP_TX_WAIT;
                Dcm_TpTxConfirmation(txConfigListItem->PduR_PduId, NTFRSLT_E_TIMEOUT_BS ); /** @req CANTP204 */ /** @req CANTP316 */
            }
            break;
        case TX_WAIT_TX_CONFIRMATION:
            ...
            if (txRuntimeListItem->iso15765.stateTimeoutCount == 0) { /** @req CANTP075 */
                ...
                txRuntimeListItem->pduId = INVALID_PDU_ID;
                txRuntimeListItem->mode = CANTP_TX_WAIT;
                Dcm_TpTxConfirmation(txConfigListItem->PduR_PduId, NTFRSLT_E_TIMEOUT_A ); /**add by xuey.04279**/	
            }
            break;
        default:
            break;
    ...
    switch (rxRuntimeListItem->iso15765.state) {
        case RX_WAIT_CONSECUTIVE_FRAME: {
            ...
            if (rxRuntimeListItem->iso15765.stateTimeoutCount == 0) {
                /* N_Cr timeout */
                Dcm_TpRxIndication(rxConfigListItem->PduR_PduId, NTFRSLT_E_TIMEOUT_CR); /** @req CANTP084 */ /** @req CANTP313 */
                rxRuntimeListItem->iso15765.state = IDLE;
                rxRuntimeListItem->pduId = INVALID_PDU_ID;
                rxRuntimeListItem->mode = CANTP_RX_WAIT;
            }
            ...
            else if (ret == BUFREQ_E_BUSY)
            {
                if (rxRuntimeListItem->iso15765.stateTimeoutCount == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Dcm_TpRxIndication(rxConfigListItem->PduR_PduId, NTFRSLT_E_NOT_OK); /** @req CANTP084 */
                        }
                    }
                }
                else
